...
const SideBar =()=>
{
    ...
    return (
                <header className="pos-r h-screen inline-flex flex-col justify-between bg-white shadow p-6" style={{marginRight:'250px'}}>
                    <nav className=" inline-flex flex-col space-y-2">
                        {
                            navbarItems.map((link,index)=>
                                <a  key={index} className="flex items-center text-gray-600 py-2 cursor-pointer hover:bg-gray-100">
                                    <span className="w-8 h-8 p-1" >
                                        {/* {selected == index ? link.selectedIcon : link.icon} */}
                                    </span>
                                    <span className="font-medium select-none" >{link.text}</span>
                                </a>
                                )
                        }
                    
                    </nav>
                    <button className="h-8 w-8 p-1 bg-gray-100 text-gray-600 rounded-lg mx-auto border border-solid border-gray-200 hover:border-gray-300" onClick={()=>isEnlarge = !isEnlarge}>
                    <span >span</span>
                    </button>
                </header>
